...
UPLOAD_FOLDER_1 = os.path.join(app.root_path, 'KPI')
UPLOAD_FOLDER_2 = os.path.join(app.root_path, 'Tasks')
...
def restructure_excel(file_path):
    """Restructure the Excel file according to business logic."""
    ...
    def sanitize_sheet_name(name):
        return str(name).replace(':', '-').replace('/', '-').replace('\\', '-').replace('*', '-').replace('?', '-').replace('[', '-').replace(']', '-')

    output_path_custom = os.path.join(app.root_path, 'kpi.xlsx')
    ...
    with pd.ExcelWriter(output_path_custom, engine='openpyxl') as writer:
        ...
        for index, row in df.iterrows():
            try:
                ...
                averages_row.to_excel(writer, sheet_name=sheet_name, startrow=len(restructured_df) + 3, header=False, index=False)
                ...
                for row in worksheet.iter_rows(min_row=3, max_row=len(restructured_df) + 3, min_col=1, max_col=worksheet.max_column):
                    for cell in row:
                ...
                for cell in worksheet.iter_rows(min_row=len(restructured_df) + 4, max_row=len(restructured_df) + 4, min_col=1, max_col=worksheet.max_column):
                    for c in cell:
                ...
                logger.error(f"An error occurred while processing row {index}: {e}")
                continue
...
def process_excel(file_path):
    """Process the Excel file according to business logic."""
    try:
        ...
        ws.insert_cols(1, 2)
        ws.cell(row=1, column=2, value="عدد المهام غير المكتملة")
        ...
        # Save the workbook to a specific path in the root directory
        output_file_path = os.path.join(app.root_path, 'processed_output.xlsx')
...
def kpi_page():
    if request.method == 'POST':
        ...
        if allowed_file(file.filename):
            file_path = save_file(file, app.config['UPLOAD_FOLDER_1'])
            try:
                if file.filename.endswith('.csv'):
                    # Convert CSV to Excel
                    converted_excel_path = convert_csv_to_excel(file_path)
                    restructured_file_path = restructure_excel(converted_excel_path)
                    os.remove(converted_excel_path)  # Remove temporary Excel file after processing
                else:
                    restructured_file_path = restructure_excel(file_path)

                return send_file(restructured_file_path, as_attachment=True)
            except Exception as e:
                logger.error(f"Error in KPI page processing: {e}")
...
def tasks_page():
    if request.method == 'POST':
        ...
        if not file or file.filename == '':
            return 'No selected file'
        if allowed_file(file.filename):
            file_path = save_file(file, app.config['UPLOAD_FOLDER_2'])
            try:
                if file.filename.endswith('.csv'):
                    # Convert CSV to Excel
                    converted_excel_path = convert_csv_to_excel(file_path)
                    processed_file_path = process_excel(converted_excel_path)
                    os.remove(converted_excel_path)  # Remove temporary Excel file after processing
                else:
                    processed_file_path = process_excel(file_path)

                if processed_file_path:
                    return send_file(processed_file_path, as_attachment=True, download_name='tasks_processed.xlsx')
                else:
                    return 'An error occurred during processing.'
            except Exception as e:
                logger.error(f"Error in tasks page processing: {e}")
                return f"An error occurred: {e}"
        else:
